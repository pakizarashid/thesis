...
src/
  ...
  eval/     disruption_pgd.py (the headline PGD result), audiopure_eval.py, audioseal_eval.py,
            false_positive_rate.py, quality_metrics.py, cross_dataset_eval.py,
...
| **`checkpoints/stage1_full_recalibrated_v3/`** | Stage 1 full, presence-calibration fix applied. **Traceability watermark + false-positive fix** | **Canonical — base for everything, including the PGD command above** |
...
**Final recommended inference pipeline (as of this writing):** `stage1_full_recalibrated_v3` (traceability + presence-calibration) with `disruption_pgd.py` applied at inference time at `epsilon=0.002, lambda_wm=1.0` (no additional checkpoint required). None of the Stage 2 or Stage 3 checkpoints are part of this final pipeline; they are retained as evidence for the negative results they document.
...
Every checkpoint collapses to chance (~50%) after purification, regardless of training approach. *(PGD-protected audio was never separately re-run through this eval — the same collapse is expected given the shared signal path, but that's an inference, not a confirmed measurement.)*
...
Lesson: the fix needed data diversity (30 vs. 10 speakers), not just more epochs — narrower data overfit instead of generalizing.

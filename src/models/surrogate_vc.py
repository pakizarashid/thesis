...
class YourTTSSurrogate(nn.Module):
    ...
    def differentiable_inference(self, text: str, d_vector: torch.Tensor, language: str = "en") -> dict:
        ...
        # Vits._set_x_lengths falls back to torch.tensor(x.shape[1:2]) --
        # ALWAYS a single-element tensor -- when x_lengths isn't explicitly
        # provided, regardless of actual batch size. This silently breaks for
        # batch_size > 1 (confirmed via source inspection after an
        # AssertionError at batch_size=2). Since every item in our batch uses
        # the identical text (same token length), compute the correct
        # per-item lengths ourselves rather than relying on that fallback.
        x_lengths_tensor = torch.full(
            (x.shape[0],), x.shape[1], dtype=torch.long, device=d_vector.device
        )
        ...
        aux_input = {"x_lengths": x_lengths_tensor, "d_vectors": d_vector, "speaker_ids": None,

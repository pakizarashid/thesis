        ...
        for batch in train_loader:
            ...
            if current_lambda > 0:
                ...
                if args.disrupt_mode == "sim":
                    # SIM-TARGETED MODE (the fix): directly minimizes speaker-
                    # embedding cosine similarity -- the SAME quantity
                    # disruption_effectiveness.py measures at eval time.
                    # Closes the proxy/target gap found in the mel-mode
                    # long-run experiment (pivotal mel-distance moved
                    # reliably, SIM did not).
                    ...
                    disrupt_loss_dict = {
                        "total": sim_loss,
                        "sim_disruption": sim_loss.detach(),
                    ...
                    # MEL MODE (original SafeSpeech-derived three-term loss) --
                    # kept available for direct comparison against sim mode.
                    disrupt_loss_dict = disruption_loss_fn(clean_audio, cloned_output)
                    disrupt_loss_dict["sim_disruption"] = torch.tensor(0.0, device=device)
            else:
                disrupt_loss_dict = {"total": torch.tensor(0.0, device=device),
                                      "pivotal_disruption": torch.tensor(0.0, device=device),
                                      "l1_to_noise": torch.tensor(0.0, device=device),
                                      "kl_to_noise": torch.tensor(0.0, device=device),
                                      "sim_disruption": torch.tensor(0.0, device=device)}
            ...
            if global_step % args.log_every == 0:
                ...
                sim_val = disrupt_loss_dict.get("sim_disruption", torch.tensor(0.0))
                print(f"[epoch {epoch} step {global_step}] mode={args.disrupt_mode} lambda={current_lambda:.4f} "
                      ...
                      f"| sim_disrupt={sim_val.item() if torch.is_tensor(sim_val) else sim_val:.4f} "
...
def build_arg_parser():
    ...
    p.add_argument("--disrupt_mode", type=str, default="sim", choices=["mel", "sim"],
                    help="'sim' (default, RECOMMENDED): directly minimizes speaker-embedding cosine "
                         "similarity -- the fix for the proxy/target mismatch found when 'mel' mode "
                         "reliably moved pivotal_distance but not SIM over a 1500-step run. "
                         "'mel': the original SafeSpeech-derived 3-term loss (pivotal + l1_to_noise + "
                         "kl_to_noise), kept for direct comparison.")

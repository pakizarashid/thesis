...
def load_stage1_checkpoint(backbone, checkpoint_path: str):
    ...
    if non_lora_missing:
        print(f"[load_stage1_checkpoint] WARNING - {len(non_lora_missing)} LoRA keys missing "
              f"(r/alpha mismatch with Stage 1 run?): {non_lora_missing[:3]}...")
...
def train(args):
    ...
    disruption_loss_fn = SafeSpeechDisruptionLoss(
        ...
        weight_beta=args.disrupt_weight_beta,
    ).to(device)
...
def build_arg_parser():
    ...
    p.add_argument("--stage1_checkpoint", type=str, required=True,
                    help="Path to a Stage 1 checkpoint to resume from (required -- see module docstring).")
    ...
    p.add_argument("--disrupt_lambda_mel", type=float, default=1.0,
                    help="Inner weight on SafeSpeechDisruptionLoss's own pivotal term.")
    p.add_argument("--disrupt_weight_beta", type=float, default=10.0,
                    help="SafeSpeech's own default -- see safespeech_losses.py docstring, likely needs retuning.")

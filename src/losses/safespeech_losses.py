...
def compute_sim_disruption_loss(emb_clean: torch.Tensor, emb_cloned: torch.Tensor) -> torch.Tensor:
    """
    Directly minimizes speaker-embedding cosine similarity between the
    original clean audio and the surrogate's cloned output. This is the FIX
    for the proxy/target mismatch found in the long-run experiment: training
    on pivotal mel-distance (compute_pivotal_disruption_loss) reliably
    increased spectral distance but did NOT reliably reduce SIM (the metric
    that actually determines anti-cloning efficacy, measured identically at
    eval time by disruption_effectiveness.py's compute_sim). This loss trains
    on the SAME quantity SIM measures, closing that gap directly.

    emb_clean, emb_cloned: [batch, embedding_dim], from
    surrogate.compute_speaker_embedding() on clean_audio and cloned_output
    respectively -- computed by the CALLER (not this function), since the
    embedding extraction needs the surrogate's speaker encoder, which this
    module doesn't own.

    NOTE ON SIGN: unlike compute_pivotal_disruption_loss (which needed
    negation in the caller, since we want to MAXIMIZE mel-distance), this
    function's raw output is minimized DIRECTLY -- no sign flip needed. Lower
    cosine similarity = less speaker-identity similarity = more disruption,
    and the training loop already minimizes total loss via gradient descent,
    so directly adding this term (unnegated) already pushes similarity down.
    """
    return F.cosine_similarity(emb_clean, emb_cloned, dim=-1).mean()
